describe("34-elseif-vs-switch-vs-mapa.js", () => {
  ...
  it("if elseif else", () => {
    ...
    // mientras funcione.
    expect(girarALaIzquierda("Norte")).toBe("Oeste");
    expect(girarALaIzquierda("Oeste")).toBe("Sur");
    expect(girarALaIzquierda("Sur")).toBe("Este");
    expect(girarALaIzquierda("Este")).toBe("Norte");
  });
  ...
  it("usando switch por primera vez", () => {
    ...
    expect(girarALaDerecha("Norte")).toBe("Este");
    expect(girarALaDerecha("Este")).toBe("Norte");
    // O.o
    // Como dije, cuando un caso coincide, sigue ejecutando hasta
    // que se encuentra un "break" o acaba el bloque de código.
    expect(girarALaDerecha("Sur")).toBe("Norte");
    expect(girarALaDerecha("Oeste")).toBe("Norte");
