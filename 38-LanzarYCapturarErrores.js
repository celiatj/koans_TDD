describe("38-LanzarYCapturarErrores.js", () => {
  ...
  it("Capturando y lanzando errores", () => {
    ...
    expect(unaVariable).toBe("el principio del try");
    expect(otraVariable).toBe("valor inicial");
    expect(otraVariableMas).toBe("Estuve en un catch");
  ...
  it("¿qué tipos existen?", () => {
    ...
    expect(unError.message).toBe("Aquí va un mensaje de error");
  });
  ...
  it("se puede capturar y reaccionar dependiendo del error", () => {
    ...
    expect(queValorAcabareTeniendo).toBe("¿O este?");
  });
  ...
  it("DIY: Coloca tu los errores", () => {
    ...
    expect(unaVariable).toBe("valor dentro del try");
    expect(otraVariable).toBe("valor dentro del try");
