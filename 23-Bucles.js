describe("23-Bucles.js", () => {
  ...
  it("while", () => {
    ...
    // Recuerdas ¿cuandó accediamos a los caracteres de una string por su posición? ^.-
  ...
  it("DIY: While edition", () => {
    ...
    const uglify = function (frase) {
      let result = "";
      const ultimaPosicionDelString = frase.length - 1;
      let posicionActual = 0;
      while (posicionActual <= ultimaPosicionDelString) {
        // const caracterActual = frase.charAt(posicionActual); // También se puede usar "[]"
        let caracterActual = frase[posicionActual];
        if (caracterActual === "-") {
          caracterActual = "_";
        }
        if (caracterActual === " ") {
    ...
    expect(uglify("una strIng coN mUchos esPacios En bLanco")).toBe(
      "una-string-con-muchos-espacios-en-blanco"
    );
    // BONUS TRACK!!!!! Cambia los '-' originales por '_' (Toma emojis ;)
    expect(uglify("El partido acabo 10-9 ¡AJUSTADISIMO!")).toBe(
      "el-partido-acabo-10_9-¡ajustadisimo!"
    );
    // Solo por asegurar ^^
    expect(
      uglify("Este partido han ganado 10-9 y el anterior quedaron empate 9-9")
    ).toBe("este-partido-han-ganado-10_9-y-el-anterior-quedaron-empate-9_9");
  });
  ...
  it("for", () => {
    ...
    for (
      let posicionActual = 0;
      posicionActual < unaString.length;
      posicionActual += 1
    ) {
  ...
  it("DIY: for edition", () => {
    ...
    function cuentaCaracteres(primeraString, segundaString) {
      let contador = 0;
      if (primeraString === undefined || segundaString === undefined) {
        return 0;
      }
      const ultimaPosicionDelString = Math.min(
        primeraString.length,
        segundaString.length
      );
      for (let index = 0; index < ultimaPosicionDelString; index++) {
        if (primeraString != undefined && segundaString != undefined) {
          if (
            primeraString[index].toLowerCase() ===
            segundaString[index].toLowerCase()
          ) {
            contador++;
          }
        }
  ...
  it("DIY: for of deluxe edition", () => {
    ...
    const noEsUnaVocal = function (character) {
      return "bcdfghjklmnñpqrstvxzwy".includes(character);
    };

    const cuentaConsonantes = function (unaString) {
      let cantidadDeConsonantes = 0;
      if (unaString != undefined) {
        for (let caracterActual of unaString) {
          if (noEsUnaVocal(caracterActual)) {
    ...
    expect(cuentaConsonantes(todasLasConsonantes + todasLasConsonantes)).toBe(
      44
    );
