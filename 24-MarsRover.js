describe("24-MarsRover.js", () => {
  ...
  const MarsRover = class {
    ...
    ejecutarOrdenes(orden) {
      if (orden != "") {
        for (const action of orden) {
          if (action === "m") {
            switch (this.miOrientacion()) {
              case "Norte":
                this.y++;
          ...
          if (action === "d") {
            this.mueveALaderecha();
          }
          if (action === "i") {
            this.mueveALaIzquierda();
          }
        }
      }
    }
  ...
  it("BONUS TRACK: ...", () => {
    ...
    expect("El bonus track para otro día, ahora a descansar ^^").toBe(
      "El bonus track para otro día, ahora a descansar ^^"
    );
  });
});
