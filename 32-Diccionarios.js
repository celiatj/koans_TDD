describe("32-Diccionarios.js", () => {
  ...
  it("se definen de la siguiente manera", () => {
    ...
    expect(unDiccionarioConUnaClaveYUnValor["unaClave"]).toBe("unValor");
    // Existe otra manera
    expect(unDiccionarioConUnaClaveYUnValor.unaClave).toBe("unValor");
    // Que sospechoso, se parece a acceder a un atributo de una clase ;)
    // ¿Qué pasa si accedemos a una clave que no está definida?
    expect(unDiccionarioVacio["claveSinDefinir"]).toBe(undefined);
    expect(unDiccionarioVacio.claveSinDefinir).toBe(undefined);
    // Sin sorpresas, ¿no?
  });
  ...
  it("puede contener cualquier valor", () => {
    ...
    expect(unDiccionario.booleano).toBe(true);
    expect(unDiccionario.integer).toBe(19);
    expect(unDiccionario.float).toBe(12.1);
    expect(unDiccionario.funcion.toString()).toBe("function () {}");
    // Pequeña trampa lo del toString() ^^
    expect(unDiccionario.clase.toString()).toBe("class {}");
    expect(unDiccionario.array).toBe([]);
    // ¿Recuerdas cómo acceder a un array de dentro de un array?
    // Si no repasa los ficheros de Arrays y saca el valor dentro del
    // diccionario :)
    expect(unDiccionario.diccionario.otraClave).toBe("otroValor");
  ...
  it("solo puede tener strings como claves", () => {
    ...
    // Sospechoso, ¿no?
    expect(diccionario["letras"]).toBe("valorDeLetras");
    expect(diccionario[0]).toBe("valorDeLetras");
    // No dudes.
    expect(diccionario[1.1]).toBe("valorDeLetras");
    ...
    expect(clavesDelDiccionario).toBe(["0", "letras", "1.1"]);
  ...
  it("puede añadir valores", () => {
    ...
    expect(unDiccionario.ortaClave).toBe("otroValor");
    expect(unDiccionario["ortaClaveMas"]).toBe("otroValorMas");
  });
  ...
  it("puede reasignar valores", () => {
    ...
    expect(unDiccionario["unaClave"]).toBe("otroValor");

    unDiccionario["unaClave"] = "otroValorMas";

    expect(unDiccionario.unaClave).toBe("otroValorMas");
  });
  ...
  it("sabe si contiene una clave", () => {
    ...
    expect(contieneLaClave).toBe(true);

    const contieneOtraClave = diccionario.hasOwnProperty("otraClave");

    expect(contieneOtraClave).toBe(false);
  });
  ...
  it("puede obtener un listado de sus claves", () => {
    ...
    expect(nombres).toBe(["Akira", "Charlie", "Harper", "Jess"]);
    ...
    expect(developers).toBe(["Akira", "Charlie", "Jess"]);
    expect(recruiters).toBe(["Harper"]);
  ...
  it("puede obtener un listado de sus valores", () => {
    ...
    expect(cantidades).toBe([50, 30, 40]);
  });
  ...
  it('puede "convertirse" en string', () => {
    ...
    expect(unaString).toBe("[object Object]");
  ...
  it("puede convertirse en JSON", () => {
    ...
    expect(string).toBe('{"unaClave":"unValor"}');
    ...
    expect(otraVezElDiccionario).toBe({ unaClave: "unValor" });
  });
