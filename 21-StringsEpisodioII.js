...
describe('21-StringsEpisodioII.js', () => {
  ...
  it('sabe la si contiene otra string', () => {
    ...
    expect(contieneLaSubString).toBe(true)

    contieneLaSubString = unaString.includes('otra string')

    expect(contieneLaSubString).toBe(false)
  ...
  it('puede reemplazar un trozo de la string por otro trozo', () => {
    ...
    expect(unaStringReemplazada).toBe('otra string cualquiera')
    ...
    expect(noContienePalabrasRepetidas).toBe('no repetida')
  ...
  it('puede transformar todos los caracteres en mayúsculas', () => {
    ...
    expect(todoEnMayusculas).toBe('ALGUNASMINÚSCULAS ^^')
    ...
    expect(estaEnMayusculas('unaString')).toBe(false)
    expect(estaEnMayusculas('OTRASTRING')).toBe(true)
  })
  ...
  it('puede transformar todos los caracteres en minúsculas', () => {
    ...
    // Fácil, ¿no?
    expect(stringEnMinusculas).toBe('string_en_mayúsculas')
  })
  ...
  it('puede concatenar strings', () => {
    ...
    expect(stringConcatenada).toBe('unaStringotraString')
  })
  ...
  it('puede trimmearse', () => {
    ...
    expect(stringTrimmeada).toBe('s t tings con e spacios')
  })
  ...
  it('sabe el carácter que tiene en una posición en concreta', () => {
    ...
    expect(unaStringCualquiera.charAt(ultimaPosicion)).toBe('0')
    // Va un poco de práctica, que en el futuro esto te será muy, muy útil.
    expect(unaStringCualquiera.charAt(3)).toBe('4')
    expect(unaStringCualquiera.charAt(6)).toBe('7')
    // ¿Qué pasará si usamos un índice que está fuera del rango?
    expect(unaStringCualquiera.charAt(123123132131)).toBe('')
  ...
  it('sabe el carácter que tiene en una posición en concreta', () => {
    ...
    const ultimaPosicion = (unaStringCualquiera.length - 1)
    expect(unaStringCualquiera[ultimaPosicion]).toBe('0')
    expect(unaStringCualquiera[3]).toBe('4')
    expect(unaStringCualquiera[6]).toBe('7')
    expect(unaStringCualquiera[123123132131]).toBe(undefined)
